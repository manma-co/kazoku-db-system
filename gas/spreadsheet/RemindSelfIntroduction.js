...
function sent_remind_self_introduction_Function(sheets, MAM_COLUMN) {
  ...
  for (var i = 0; i < data.length; ++i) {
    ...
    if (self_intro_1 === "") {
      mail_self_introduction(row[MAM_COLUMN.STUDENT_EMAIL_1], row[MAM_COLUMN.STUDENT_NAME_1])
    }

    // 2人目
    var self_intro_2 = row[MAM_COLUMN.SELF_INTRO_2];
    if (self_intro_2 === "") {
      mail_self_introduction(row[MAM_COLUMN.STUDENT_EMAIL_2], row[MAM_COLUMN.STUDENT_NAME_2])
    }

    // 3人目
    var self_intro_3 = row[MAM_COLUMN.SELF_INTRO_3];
    if (self_intro_3 === "") {
      mail_self_introduction(row[MAM_COLUMN.STUDENT_EMAIL_3], row[MAM_COLUMN.STUDENT_NAME_3])
    }
  }
}
...
function mail_self_introduction(student_mail, student_name) {
  if (student_mail === "" || student_name === "") {
    return
  }
  Logger.log(student_mail + "に送信")
  GmailApp.sendEmail(student_mail, get_subject_self_introduction(), get_message_self_introduction(student_name), {name: 'manma'})
}

function get_subject_self_introduction() {
  return "【要確認】自己紹介メール送信のお願い";
}

function get_message_self_introduction(student_name) {
